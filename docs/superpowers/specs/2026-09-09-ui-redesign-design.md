...
| `ModelRow` | Two columns, no capsules (amended 2026-09-09 after the second live test: three chips per row made the list read as a form). Left: radio, name in `bodyMedium`, "Recommended" as accent `caption` text, purpose note under them. Right, right-aligned: size as plain `valueSmall` `muted` text, the quality bar as five 4 pt dots in Settings only, then the status — "Downloaded" with a check in `ok`, or the Download button (`.dsPrimary` on the selected row, `.dsSecondary` elsewhere). A running or failed transfer takes a full-width line under both columns; nothing else does. Selected = `accentSoft` fill + `accent` border, highlighted = `warn` border, otherwise a `line` hairline and no fill. `isCompact` tightens it for onboarding | `ModelRow(name:note:isSelected:isDownloaded:download:onSelect:)` |
...
- **Recognition:** card "Language" with the existing 12-entry menu picker and its note, then card "Model" with `ModelRow` per model (Base, Small, Large Turbo Q5 *recommended*, Large Turbo Q8, Large Turbo), each carrying its one-line `ModelSize.purpose` note under the name; selecting a model that is not on disk shows `DownloadProgress` inside its row. This is the only section allowed to scroll: its scroll area runs to the window's bottom edge — no trailing margin, a 1 pt `line` hairline closing it — and its indicators are always visible. (Amended 2026-09-09: Language used to sit below five model rows, where it needed a scroll to reach, and the section stopped 28 pt short of the edge.)
